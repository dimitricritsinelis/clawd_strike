...
varying vec3 vFloorWorldPos;`,
  ...
  if (!shader.vertexShader.includes("vFloorWorldPos = worldPosition.xyz;")) {
    shader.vertexShader = shader.vertexShader.replace(
      "#include <worldpos_vertex>",
      `#include <worldpos_vertex>
vFloorWorldPos = worldPosition.xyz;`,
    );
  }
}
...
float floorMacroNoise(vec2 worldXZ) {
  ...
  return mix(low, high, 0.35);
      ...
      shader.fragmentShader = shader.fragmentShader.replace(
        "#include <common>",
        `#include <common>
uniform float uFloorAlbedoBoost;${macroHeader}`,
      );
    }
...
roughnessFactor = clamp(
  ...
  const cacheKey = macroEnabled
    ? `floor-tweak:${albedoBoost.toFixed(3)}:${macroColorAmplitude.toFixed(3)}:${macroRoughnessAmplitude.toFixed(3)}:${macroFrequency.toFixed(4)}:${macroSeed.toFixed(0)}`
    : `floor-tweak:${albedoBoost.toFixed(3)}:flat`;

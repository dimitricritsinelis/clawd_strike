...
async function createTempRoot(): Promise<string> {
  return mkdtemp(path.join(os.tmpdir(), "clawd-strike-agent-export-"));
}
...
test("dry-run export succeeds against a separate starter repo checkout", async () => {
  ...
  try {
    ...
    const result = await exportAgentStarterRepo({
      repoRoot: REPO_ROOT,
      outDir: targetRoot,
      dryRun: true,
...
test("bootstraps a missing target path as a separate git repo and exports deterministically", async () => {
  ...
  try {
    const targetRoot = path.join(tempRoot, PUBLIC_AGENT_COMPANION_REPO_NAME);
    const first = await exportAgentStarterRepo({
      repoRoot: REPO_ROOT,
      outDir: targetRoot,
    });
    ...
    const second = await exportAgentStarterRepo({
      repoRoot: REPO_ROOT,
      outDir: targetRoot,
    });
...
test("refuses targets nested inside the game repo", async () => {
  const nestedTarget = path.join(REPO_ROOT, "output", "agent-starter-nested-test");
  await assert.rejects(
    exportAgentStarterRepo({
      repoRoot: REPO_ROOT,
      outDir: nestedTarget,
      dryRun: true,
    }),
...
test("refuses existing non-git export targets", async () => {
  const tempRoot = await createTempRoot();
  try {
    const targetRoot = path.join(tempRoot, "starter-no-git");
...
test("expect-origin guard rejects mismatched starter repo remotes", async () => {
  const tempRoot = await createTempRoot();
  try {
    const targetRoot = path.join(tempRoot, PUBLIC_AGENT_COMPANION_REPO_NAME);
